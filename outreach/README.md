...
### 送信機能の安全性について（監査で修正した点）

送信機能の追加後、承認ゲート回避・認証情報漏洩・SMTPプロトコル・状態管理の4観点で
敵対的なレビューを行い、確認された欠陥を修正しました。

1. **SMTP認証に失敗するとパスワードが漏れる（重大）**
   アプリパスワードを打ち間違えた場合などに、パスワードのbase64がエラー文へ埋め込まれ、
   標準エラーと `outreach/audit.log.jsonl`（git管理下）に残っていました。
   → 例外生成時にも伏字を適用し、監査ログへ書く前にも伏字化を通すよう二重に対策しました。

2. **宛先の束縛が効かない場合がある（重要）**
   承認記録に `approvedRecipient` が無い場合（旧バージョンが作った記録など）、
   宛先のチェックが丸ごとスキップされ、任意のアドレスへ送信できていました。
   → 宛先の記録が無い承認は無効として扱い、必ず再承認を求めるようにしました。

3. **状態ファイルに排他制御がない（重大）**
   同時に複数実行すると `state.json` の上書きで「送信済み」の記録が消え、
   同じ相手へ二重送信できてしまう状態でした。
   → 全コマンドをロックで直列化し、書き込みを一時ファイル＋renameのアトミック方式にしました。

いずれも修正後に、指摘された再現手順をそのままなぞって解消を確認しています。
